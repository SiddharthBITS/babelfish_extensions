...
DECLARE
    exception_message text;
BEGIN
    ALTER FUNCTION sys.babelfish_conv_helper_to_time(TEXT, BOOL, NUMERIC) RENAME TO babelfish_conv_helper_to_time_with_arg_text_deprecated_in_5_1_0;
EXCEPTION
...
DECLARE
    exception_message text;
BEGIN
    ALTER FUNCTION sys.babelfish_conv_helper_to_time(ANYELEMENT, BOOL, NUMERIC) RENAME TO babelfish_conv_helper_to_time_with_arg_anyelement_deprecated_in_5_1_0;
EXCEPTION
    WHEN undefined_function THEN
        GET STACKED DIAGNOSTICS
        exception_message = MESSAGE_TEXT;
...
DECLARE
    exception_message text;
BEGIN
    ALTER FUNCTION sys.babelfish_conv_helper_to_datetime(TEXT, BOOL, NUMERIC) RENAME TO babelfish_conv_helper_to_datetime_deprecated_in_5_1_0;
EXCEPTION
    WHEN undefined_function THEN
        GET STACKED DIAGNOSTICS
        exception_message = MESSAGE_TEXT;
...
BEGIN
    IF (NOT EXISTS(SELECT * FROM pg_proc
                            WHERE proname = 'babelfish_conv_helper_to_time_with_arg_text_deprecated_in_5_1_0' AND
...
BEGIN
    IF (NOT EXISTS (SELECT * FROM pg_proc
        ...
        NOT EXISTS (SELECT * FROM pg_proc
                            WHERE proname like 'babelfish_conv_helper_to_datetime_deprecated_in_5_1_0' AND
                            pronamespace = 'sys'::regnamespace::oid))
    THEN
